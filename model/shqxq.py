...
class Export:
    ...
    def shqxq(self, start_date, end_date, dir_name):
        ...
        # '券1标签', '券1 领券数/用券数', '券2标签', '券2 领券数/用券数','券3标签', '券3 领券数/用券数',
                   #'券4标签', '券4 领券数/用券数'
        all_data = []
        ...
        sql1 = """SELECT b.merchant_id,a.subbranch_id,b.short_name,a.ADMIN_REGION_CODE,a.MICRO_REGION_CODE,
                    ...
                    FROM subbranch a,merchant b, merchant_industry c 
                    WHERE a.merchant_id=b.merchant_id AND b.merchant_type = c.merchant_type and a.sub_type=1 """
        ...
        for _sub in sql1result:
            ...
            if _sub:
                ...
                sql2 = """SELECT subbranch_id from subbranch WHERE merchant_id='{0}'""".format(_sub[0])
                sql2result = self.connect.query(self.connect.fenqi, sql2)#得到每个商户的所有门店id，再来计算每家店的流水
                #print("商户是{0}，门下的门店的是{1}".format(_sub[0],sql2result))
